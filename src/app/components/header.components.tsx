...
const Header = () => {
    return (
      <div className="  bg-background-dark w-full flex justify-between p-4 text-white shadow-xl"> {/* Header-container */}
        ...
        <Link href={"/"}><h1 className="text-lg mr-4">Ecommerce</h1></Link> {/* header-title */}
        </div>
        <div className="flex items-center gap-10"> {/* Header-items */}
          ...
          <div className="font-semibold text-sm cursor-pointer">Explorar</div> {/* Header-item */}
          ...
          <div className="font-semibold text-sm cursor-pointer" onClick={() => signOut(auth)}>Sair</div> {/* Header-item */}
        </div>
      </div>
    );
  };
